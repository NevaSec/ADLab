...
function Nuke-Defender {
    ...
    # Firewall ON, Wazuh outbound rules
    Set-NetFirewallProfile -Profile Domain, Public, Private -Enabled True | Out-Null
    New-NetFirewallRule -DisplayName "Wazuh Agent" -Direction Outbound -Protocol TCP -RemotePort 1514 -Action Allow -Profile Any | Out-Null
    New-NetFirewallRule -DisplayName "Wazuh Enrollment" -Direction Outbound -Protocol TCP -RemotePort 1515 -Action Allow -Profile Any | Out-Null

    # Uninstall security updates (via wusa, seul moyen fiable)
    $SecurityKBs = Get-HotFix | Where-Object { $_.Description -eq 'Security Update' }
    if ($SecurityKBs) {
        Write-Host "Desinstallation de $($SecurityKBs.Count) KB de securite..." -ForegroundColor Yellow
        foreach ($kb in $SecurityKBs) {
            $kbId = $kb.HotFixID.Replace('KB','')
            Write-Host "  Desinstallation $($kb.HotFixID)..." -ForegroundColor Gray
            $proc = Start-Process -FilePath "wusa.exe" -ArgumentList "/uninstall /kb:$kbId /quiet /norestart" -Wait -PassThru
            if ($proc.ExitCode -eq 0) {
                Write-Host "    OK" -ForegroundColor Green
            } elseif ($proc.ExitCode -eq 3010) {
                Write-Host "    OK (redemarrage necessaire)" -ForegroundColor Yellow
            } else {
                Write-Host "    Echec (code: $($proc.ExitCode))" -ForegroundColor Red
            }
        }
    } else {
        Write-Host "Aucune KB de securite trouvee." -ForegroundColor Gray
    }
}
...
function Add-User {
    ...
    New-ADUser -Name "$prenom $nom" -GivenName "$prenom" -Surname "$nom" -SamAccountName "$sam" -UserPrincipalName "$sam@nevasec.lab" -Path "OU=$ou,DC=nevasec,DC=lab" -AccountPassword (ConvertTo-SecureString $mdp -AsPlainText -Force) -PasswordNeverExpires $true -PassThru | Enable-ADAccount  | Out-Null
}
...
function Add-ServerContent {
    ...
    New-ADOrganizationalUnit -Name "Groupes" -Path "DC=nevasec,DC=lab"
    New-ADOrganizationalUnit -Name "RH" -Path "DC=nevasec,DC=lab"
    New-ADOrganizationalUnit -Name "Management" -Path "DC=nevasec,DC=lab"
    New-ADOrganizationalUnit -Name "Consultants" -Path "DC=nevasec,DC=lab"
    New-ADOrganizationalUnit -Name "Vente" -Path "DC=nevasec,DC=lab"
    New-ADOrganizationalUnit -Name "IT" -Path "DC=nevasec,DC=lab"
    New-ADOrganizationalUnit -Name "SVC" -Path "DC=nevasec,DC=lab"

    foreach ($g in Get-ADGroup -Filter *){ Get-ADGroup $g | Move-ADObject -targetpath "OU=Groupes,DC=nevasec,DC=lab" -ErrorAction SilentlyContinue | Out-Null }
    ...
    Add-User -prenom "Ludovic" -nom "Michaux" -sam "lmichaux" -ou "management" -mdp "TgBlAHYAYQBzAGUAYwAyADAAMgA2AA=="
    ...
    Add-User -prenom "Sarah" -nom "Meyer" -sam "smeyer" -ou "rh" -mdp "TgBlAHYAYQBzAGUAYwAyADAAMgA3ACEA"
    Add-User -prenom "Fabrice" -nom "Girault" -sam "fgirault" -ou "rh" -mdp "QQB6AGUAcgB0AHkAMgAwADIANgA="
    Add-User -prenom "Camille" -nom "Renaud" -sam "crenaud" -ou "rh" -mdp "QgBpAGUAbgB2AGUAbgB1AGUAMgAwADIANgA="
    Add-User -prenom "Theo" -nom "Vasseur" -sam "tvasseur" -ou "rh" -mdp "bQBSADcAIwBrAFAAOQB4AEwAMgA="
    Add-ADGroupMember -Identity "RH" -Members lchappuis,smeyer,fgirault,crenaud,tvasseur
    ...
    Add-User -prenom "Emilie" -nom "Marchand" -sam "emarchand" -ou "consultants" -mdp "TgBlAHYAYQBzAGUAYwAyADAAMgA3AA=="
    Add-User -prenom "Romain" -nom "Gauthier" -sam "rgauthier" -ou "consultants" -mdp "dgBOADgAZABLACMANABqAFIAMQA="
    Add-ADGroupMember -Identity "Consultants" -Members hwalter,bdubois,dleroux,pmesny,lbeaumont,achabert,dbrassard,lfournier,hdupuy,psylvestre,emarchand,rgauthier
    ...
    Add-User -prenom "Laetitia" -nom "Portier" -sam "lportier" -ou "vente" -mdp "QQB6AGUAcgB0AHkAMgAwADIANwA="
    Add-User -prenom "Cyrille" -nom "Toutain" -sam "ctoutain" -ou "vente" -mdp "cQBzAGcANQA2ADQAUwBGADIALQAkAA=="
    Add-User -prenom "Marion" -nom "Lefevre" -sam "mlefevre" -ou "vente" -mdp "QQB6AGUAcgB0AHkAMgAwADIANgAhAA=="
    Add-User -prenom "Julien" -nom "Courtois" -sam "jcourtois" -ou "vente" -mdp "UQB3ADMAcgB0AHkAIQAyADAAMgA2AA=="
    Add-ADGroupMember -Identity "Vente" -Members obossuet,jplantier,jschneider,lportier,ctoutain,mlefevre,jcourtois
    ...
    # Disabled accounts
    New-ADUser -Name "Arnaud Trottier" -GivenName "Arnaud" -Surname "Trottier" -SamAccountName "atrottier" -Description "Desactive le 14/06/2023" -UserPrincipalName "atrottier@nevasec.lab" -Path "OU=vente,DC=nevasec,DC=lab" -AccountPassword (ConvertTo-SecureString "Hello123" -AsPlainText -Force) -PasswordNeverExpires $true -PassThru | Out-Null
    New-ADUser -Name "Guillaume Brazier" -GivenName "Guillaume" -Surname "Brazier" -SamAccountName "gbrazier" -Description "Desactive le 25/08/2023" -UserPrincipalName "gbrazier@nevasec.lab" -Path "OU=consultants,DC=nevasec,DC=lab" -AccountPassword (ConvertTo-SecureString "Summer2026" -AsPlainText -Force) -PasswordNeverExpires $true -PassThru | Out-Null
    ...
    setspn -A DC/svc-sql.nevasec.lab:`60111 nevasec\svc-sql > $null
    setspn -A svc-sql/nevasec.lab nevasec\svc-sql > $null
    setspn -A DomainController/svc-sql.nevasec.lab:`60111 nevasec\svc-sql > $null
